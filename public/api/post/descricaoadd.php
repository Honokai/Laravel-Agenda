...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if($_POST['tipo'] == 1 ){
        ...
        if($row == 1 ){
            ...
            echo json_encode(array("erro"=>$resultado->errorInfo(),"query"=>$query));
        }
    }elseif($_POST['tipo'] == 2){
        if($descricao == ""){
            $query = "DELETE FROM feedback where `agenda_id`=".$id;
            $resultado = $conexao->prepare($query);
            $resultado->execute();
            $row = $resultado->rowCount();
            if($row == 1 ){
                echo json_encode(array("resposta"=>"Descrição EXCLUÍDA com sucesso."));
            } else {
